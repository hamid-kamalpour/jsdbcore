...
(function (undefined) {
	...
	var IndexedDb = {
		...
		Class : function () {
			...
			this.open = function(config) {
				...
				/** Chrome deprecated method **/
				if(window.webkitIndexedDB) {
					/** Create a request to open db **/
					var request = IndexedDB.open(config.name);
					
					/** In case of error on open request **/
					request.onerror = function (e) {
						if(config.onOpenError)
							config.onOpenError(e);
					};
					
					/** On success pass the config options  **/
					request.onsuccess = function (e) {
								
						/** version passed and db requested instance **/
						var version = config.version;
						db = e.target.result;
						var settingVersion = false;
						
						/** Case the db version is different to the passed version, update the structure **/
						if(db.version != version) {
							...
							versionRequest.onsuccess = function (e) {
								...
								/** Remove stores passed in config.removeStores object **/
								...
								for(var i in config.stores) {
									...
									/** If the flag overwrite is true, delete the old store **/
									if(store.overwrite)
										if(db.objectStoreNames.contains(store.name))
											db.deleteObjectStore(store.name);
						
									/** Create the store if not exist **/
									var objectStore = db.createObjectStore (
										store.name,
										store.options
									);
									
									/** Create the index refferent the current created store, passad in store options **/
								...
								/** Calls after the database is open **/
						...
						var settingVersionInterval(function () {
	...
	var Model = {
		...
		Class : function (config) {
			...
			var currentDatabase = JSDBCoreObject.getDbInstance(config.database);
			this.config = config;
	...
	var JSDBCoreObject = null;
	var _JSDbCore = window.JSDbCore;
	
	var JSDbCore = window.JSDbCore = {

		New : function (config) {
			JSDBCoreObject = new this.Class(config);
			return JSDBCoreObject;
		},
		
		Class : function (config) {
			
			if(!config)
				config = JSDbCore.defaultConfig;
			...
			for(i in config.db) {
				...
				this.databases[config.db[i].name] = Database.New(config.db[i]);	
			
			}
			
			this.noConflict = function (newName) {
				
				window[newName] = JSDbCore;
				window.JSDbCore = _JSDbCore;
